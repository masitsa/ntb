		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				else if($post_comment_status == 1)
				{
					...
					$button = '<a class="btn btn-info" href="'.site_url().'deactivate-comment/'.$post_comment_id.'/'.$post_id.'" onclick="return confirm(\'Do you want to deactivate '.$post_comment_user.' comment?\');">Deactivate</a>';
				}
