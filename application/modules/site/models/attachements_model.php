...
class Attachments_model extends CI_Model 
{
	...
	*
	*/
	public function edit_attachments($attachments_id,$meeting_id) 
	{
		//initialize required variables
		$v_data['assigned_to_error'] = '';
		$v_data['priority_status_id_error'] = '';
		...
		$v_data['priority_status_query'] = $this->attachments_model->get_priority_statuses();
		$v_data['action_status_query'] = $this->attachments_model->get_action_statuses();
		$attachments_query = $this->attachments_model->get_attachments($attachments_id);
		...
		if ($this->form_validation->run())
		{
			if($this->attachments_model->edit_attachments($attachments_id))
			{
		...
		else
		{
			...
			{
				if($attachments_query->num_rows() > 0)
				{
					$row = $attachments_query->row();
					...
					$v_data['attachments_notes'] = $row->attachments_notes;
				}
	...
	*
	*/
	public function delete_attachments($attachments_id,$meeting_id)
	{
		//delete action point
		if($this->attachments_model->delete_attachments($attachments_id))
		{
